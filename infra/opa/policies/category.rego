...
# — so this only OPENS the gate, never widens the rows. Since TAG-GATED-CREATE (ADR 0034) it is scoped to
# the LIST verb — the only type-level verb whose rows are cut afterwards; every other type-level verb
# goes through the PLACEMENT GATE below. Opt-in/default-off via the same data.category.inheritable; a
# true stranger (no role / no inheritable grant) is still denied.
allow if {
...
# the catalog for a top-level category, the parent category otherwise; manager-enriched, three-state like root_attributes:
...
# The verb is granted on the decided type directly (a role naming `category` itself) …
...
# type. It is the whole coarse LIST gate above and one of the two verb paths of the placement gate (so a
# member with WRITE/TAG on the catalog can create/tag categories whose placement and payload match its
# tags); a non-member resolves no role and is denied. The "list" in the name is historical (Phase 5.5-B).
