 ...
 * @param action     the action verb, from the shipped permission vocabulary ({@code list}, {@code view})
 * @param category   the permission category the verb expands from (e.g. {@code READ})
 * @param targetType the resource type the tool reads ({@code catalog}, {@code category},
 *     {@code product}). This is what lets the tool-gate derive the principal's ceiling through the
 *     <strong>shipped</strong> role model — {@code permissions.effective_actions(role_definition,
 *     targetType)} — instead of inventing a parallel "tool" permission key. It is also the type the
 *     catalog service will independently gate downstream, which is what makes the two-layer
 *     composition provable rather than merely asserted.
 * @param riskTags   at least one risk tag; compared against an agent capability's max risk tag
 */
public record ToolDescriptor(
        String name, String action, String category, String targetType, Set<String> riskTags) {

    public ToolDescriptor {
        name = requireText(name, "name", name);
        action = requireText(action, "action", name);
        category = requireText(category, "category", name);
        targetType = requireText(targetType, "target type", name);
