...
/**
 * {@link ProductListAuthorizer} maps a role-source outage to a fail-closed empty page — the
 * {@link CategoryListAuthorizerOutageTest} mirror for the product list (taggable products): the
 * {@link RoleResolutionException} never escapes as a 500 and the query service is never called
 * (the outage never reaches the filter / OPA).
 */
class ProductListAuthorizerOutageTest {
    ...
    private final ProductListAuthorizer authorizer =
            new ProductListAuthorizer(products, supplier, queryServiceProvider, subtreeProvider);
