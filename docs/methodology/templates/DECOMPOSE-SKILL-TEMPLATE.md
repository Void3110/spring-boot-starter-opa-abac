...
| 5 | Scaffold | `scripts/planning/scaffold-package.py` — idempotent, `--force` to overwrite, `--with-design` for the phase-① stubs, `--parts "<declaration>"` to write an execution-parts line **verbatim** (it validates nothing — slot 6's checker is the single authority), `--planning-root <dir>` for out-of-tree fixture skeletons | A deterministic scaffold script (folder + stubs + valid frontmatter) beats copying a shipped package by hand |
| 6 | Verify gates (mechanical) | `scripts/planning/verify-package.sh` — files, frontmatter, clean-room scan (private blocklist in a gitignored `.local`, fail-closed), no unfilled «slots», prompt invariants, count match, citation cross-reference (`check-citations.py`), wikilink resolution, execution-parts coverage (`check-parts.py` — a declared partition must cover exactly T1..TN; a near-miss is an error, never "absent"; absence is the green single-session default) | Keep the base four + citations; swap clean-room for repo-specific scans; commit the script, keep the blocklist out of the public repo |
