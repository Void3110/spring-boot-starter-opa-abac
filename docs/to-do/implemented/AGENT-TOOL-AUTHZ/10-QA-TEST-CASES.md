...
| I6 | `AgentCapabilitySupplier` over its real HTTP edge (`HttpServer` stub): 200-with-profile / 200-empty / 5xx / timeout / malformed body | the tri-state holds **at the wire**: resolved → narrow; authoritative-empty → deny-all; the last three → **deny** with the outage code (the [[HTTP-RESILIENCE]] contract). **WAIVED 2026-07-31 (deep review) — unimplementable as written:** the shipped supplier is `ConfigAgentCapabilitySupplier`, a Spring-config source with *no HTTP edge*, so there is no wire for this case to exercise. The tri-state it was written to protect is covered where it actually lives — `AgentCapabilitySupplierTest` (resolved / authoritative-empty / outage) and **U15** for the deny path. Re-instate this case verbatim if a remote capability source is ever added. | T3 |
| I7 | The exact JSON sent to the OPA stub for one agent tool call | the body carries `subject.attributes.actor`, `.chain`, `.agent_capability`, `resource.type == "tool"` with the declared category/risk-tags, and the principal's ceiling in the **shipped** fields — proving the shape is **additive** ([[00-DESIGN]] §2.2), with no key removed or renamed. *(Implemented 2026-07-31 by the deep review, which found it cited but never written: `ToolCallGateTest.sendsTheDeclaredToolAttributesAndTheDualIdentityToThePolicy` captures the stub's request body — a gate that asks the wrong question can still get the right answer from a stub.)* | T3 |
