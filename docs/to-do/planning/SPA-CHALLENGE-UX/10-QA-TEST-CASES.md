...
> left in drill state advertises `max_age=5` to every later cell); and **response rewriting**
> = a `window.fetch` patch installed in the page (T6, measured): wrap `fetch`, and on the response
> either rewrite the `WWW-Authenticate` header or edit the JSON body before handing it back. The app
> reads the header at `api.ts`'s `parseChallenge(res.headers.get(...))` and the body through the same
> `request()`, so the patch exercises the **real** app code path. This **replaces** the original
> DevTools **Local Overrides** recipe, which the Browser pane cannot drive — the pane can, and does,
> drive this one. The envelope key is `items` (not `content`). Cells are run **adversarially**: the
...
| E17 | Badge-vs-server honesty (the adversarial cell) | make the prediction wrong on purpose and check the truth wins: (a) as `sup-demo` elevated, let the window lapse so the chip is amber, then open `…0002` **inside the policy's skew** — the server may still answer 200: the UI shows the contents and the chip stays amber (prediction, honestly labeled); (b) strip `env` from the catalog body (the `window.fetch` patch) so no amber shows, open `…0002` at `aal1` → the panel still renders (the server's 401 is the truth); (c) a catalog whose `_provenance` is absent (the same patch, deleting the key from every `items[]` entry) shows **no** supervised badge and **no** amber, and its contents behave as the server decides; (d) the grid as `sup-demo` shows the **supervised** badge on both demo catalogs and the amber **production · verify to open** chip on `…0002` only while not elevated | T4 |
| E18 | The parser negatives on the wire | with the `window.fetch` patch, rewrite `WWW-Authenticate` on the real 401 and check the UI against the **parser's spec**, not a blanket expectation. Seven must degrade to a **plain** 401 error box (never a broken [Verify] panel): drop `max_age`; drop `acr_values`; scheme `Bearer`→`Basic`; an unterminated quoted-string; `error_description` unquoted so its spaces break the grammar; `max_age="30.5"`; `max_age="-1"`. One must **still render the panel**: a backslash-escaped quote inside `error_description` is a legal RFC 7235 quoted-pair, so it parses and the description renders **unescaped** — this is the defensive branch in `parseAuthParams`, and asserting it is refused would be asserting a bug. Removing the patch → the genuine panel returns. Confirms U1's defensive cases + U2 end to end | T3 |
