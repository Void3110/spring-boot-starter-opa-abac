...
- Phase 7 (pre-publish) **user-directory work, split into two slices** (grill-me 2026-07-06): **Slice 1** [[DIRECTORY-QUERY-FILTERS]] — **decomposed, ready for the autonomous run** (5 tickets: `?subject`/`?targetType`+`?targetId` filters that kill the SPA's `listAll*` page-walks, the 204/`Accept` `produces` fix, the bootstrap displayName upsert); **Slice 2** [[USER-DIRECTORY-PORT]] — **decomposed, ready for the autonomous run** (6 tickets: the `UserDirectory` search SPI in the library, a `KeycloakUserDirectory` in a new optional module, starter auto-config, the bearer-only `search` endpoint under `/api/v1/users`, the least-privilege `view-users` realm client, e2e + SPA picker rewrite; pinned by [[0020-user-directory-port|ADR 0020]])
