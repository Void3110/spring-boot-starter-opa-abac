...
> **Route to publish (settled 2026-06-13; B3 inserted 2026-06-15).** With 6.5 merged (PR #34), the
> order to Phase 7 is **B2 fix-slice → Phase 6.7 → Phase 6 → B3 resilience → Phase 7 (publish)** — this
> **reorders the old `6 → 6.7`** and adds **B3** before publish:
> 1. **B2 fix-slice** *(standalone, first — ✅ design settled 2026-06-15)* — the supplier-outage
>    error-distinct posture. A resolve outage currently rides the realm-role fallback and, post-6.5,
>    **erases** the resolved role's `denied_actions`/`required_tags` narrowing (the one tracked
>    widening-on-failure, [[PERMISSION-MODEL]] / the 6.5 review). Fixed on its own small branch —
>    distinguish *outage → throw → deny* from *no-role → `empty` → fallback* at the SPI contract — kept
>    standalone (its own design + review) so the security fix isn't entangled with 6.7's larger taxonomy
>    work. Pinned by ADR [[0014-supplier-outage-error-distinct|0014]] + [[B2-SUPPLIER-OUTAGE]] (00-DESIGN);
>    ready for `/decompose`.
> 2. **Phase 6.7 — control-plane vocabulary** *(the last correctness slice before publish)* — closes
>    the `define-tags` **enforcement** deferral (Epic G/Story G4: ships in the expansion math only
>    today) and categorizes the `team:*` control plane. Needs its own phase-① (the escalation analysis
>    for categorized control-plane power is a different problem from the catalog-domain taxonomy).
> 3. **Phase 6 — action enrichment** *(after 6.7, by maintainer call)* — a fuller feature story before
>    publish (affordance metadata is the headline differentiator). Independent of 6.7; consumes 5.97
>    attributes + the 6.5 vocabulary.
> 4. **Slice B3 — cross-service HTTP resilience** *(availability hardening, before publish)* — the
>    deliberate availability pass that softens the **outage → hard-deny wall** B2 introduces. Scoped to
>    *all* cross-service HTTP edges (`HttpRoleDefinitionSupplier`, `TagDefinitionClient`, `OpaClient`):
>    a uniform retry/backoff/circuit-break posture that classifies transient (5xx/timeout) vs permanent
>    (4xx) **without** weakening B2's outage→deny contract. Its own phase-①/decompose/review.
...
| **6.7** | **Control-plane vocabulary categorization** | Extend the category model to the control plane: the `team:*` verbs (`manage`, `define-roles`, `define-tags`, `transfer-ownership`), the resolve-side team-verb injection ladder, and `TeamRoleCapabilities` — re-thought against the 6.5 taxonomy, incl. whether custom roles may carry team verbs and where the tag-dictionary endpoints re-gate (the `define-tags` enforcement 6.5 defers here). | 🔜 **Route step 2 — the last correctness slice before publish (settled 2026-06-13).** Runs after the B2 fix-slice. Closes the `define-tags` **enforcement** deferral (Epic G/Story G4 — ships in the expansion math only today). Needs its own phase-① cycle: the escalation analysis for categorized control-plane power is a different problem from the catalog-domain taxonomy. Inputs: ADR 0007 + the 6.5 addendum; [[PERMISSION-CATEGORIES]] decisions 1/5/8 (the deferred surfaces). |
