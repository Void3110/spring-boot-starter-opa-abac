...
- Phase 9 **decomposed** (2026-07-28): **agent tool-call authorization** — a dual-identity (principal ≠ actor) PDP/PEP for MCP/agent tool invocations built on the starter. Design settled and pinned by [[0028-agent-tool-call-authorization|ADR 0028]] (two-layer decision model; enforcement by composition — nothing asserted downstream; agent capability narrows only). Planning package ready to run: [[AGENT-TOOL-AUTHZ]] (6 tickets, a new `example-mcp-server` on Spring AI; no library module or existing service touched — the optional `opa-abac-agent` module is the slice's exit criterion)
