...
[[HIERARCHY-SINGLE-RESOURCE]]. **5.5-B (hierarchy-aware list filter)** is still planned. Generalizes the
...
the hierarchy-aware list filter. Pinned by ADR [[adr/0008-hierarchical-resource-authorization|0008]]; stories
in [[USER-STORIES]] (Epic H). Per-node independent grants (a mid-tree node with its own team) are deferred to
