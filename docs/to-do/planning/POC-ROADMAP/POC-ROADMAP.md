...
> **reorders the old `6 → 6.7`** and adds **B3** before publish. **Status (2026-06-17): B2, 6.7, and
> Phase 6 (action enrichment) are all ✅ shipped; B3 (cross-service HTTP resilience) is now the front of
> the queue.** Remaining order: **B3 → Phase 7.**
...
> 4. **Slice B3 — cross-service HTTP resilience** *(availability hardening, ◀ NOW NEXT before publish —
>    **design settled 2026-06-18, ready for /decompose**)* — the deliberate availability pass that softens
>    the **outage → hard-deny wall** B2 introduces. Scoped to *all* cross-service HTTP edges
>    (`HttpRoleDefinitionSupplier`, `TagDefinitionClient`, `OpaClient`): a uniform retry/backoff/
>    circuit-break posture that classifies transient (5xx/timeout) vs permanent (4xx) **without**
>    weakening B2's outage→deny contract. Phase-① done — ADR [[0017-cross-service-http-resilience|0017]] +
>    [[B3-HTTP-RESILIENCE]] (10 forks pinned: decorator-over-`OpaClient`, fail-closed-in-every-state,
>    side-effect-free retry, asymmetric per-edge budgets, 3 per-endpoint breakers, optional/conditional
>    R4j, the backend-agnostic `CallGuard` seam, per-edge kill-switch). Decompose/review next.
...
| **B3** | **Cross-service HTTP resilience** *(availability; before publish)* | The deliberate availability pass that softens the **outage → hard-deny wall** Slice B2 introduces. A **uniform** retry/backoff/circuit-break posture across *all* cross-service HTTP edges — `HttpRoleDefinitionSupplier`, `TagDefinitionClient`, and the `OpaClient` HTTP hop — that classifies **transient** (5xx / timeout / connection-refused → retry) vs **permanent** (4xx → fail fast) and bounds latency (esp. on request-handling/under-lock paths). **Must preserve B2's contract**: an exhausted-retry outage still throws `RoleResolutionException` → deny; resilience only makes outages *rarer*, never re-opens the fallback. | 🔜 **Design settled (grill-me 2026-06-18) — ready for /decompose.** ADR [[0017-cross-service-http-resilience\|0017]] + [[B3-HTTP-RESILIENCE]] (+ its 00-DESIGN: ten forks, behavior matrix, all-edges sweep, proof obligations). Pinned: OPA resilience = a **decorator over the existing `OpaClient` interface** (no pluggable transport in core — the interface already provides it); fail-closed contract **identical in every breaker/config state** (`compile`→`error()` with `fromError=true`, never `denyAll()`/`allowAll()` — the 5.5-B widening landmine); **side-effect-free** retry (incl. read-timeout; future mutating edges opt out); **asymmetric per-edge budgets** ("uniform" = classification+config+contract, *not* numbers — OPA gate 1 retry, resolve+tag 2; **no resilience under the team-row `FOR UPDATE`**); **three breakers** per-endpoint, **breaker outcome-invariance** (latency/load only, never a decision input); library ships OPA resilience via **optional/conditional Resilience4j**; a thin backend-agnostic **`CallGuard` seam** (R4j now, Boot-4 native later — **two artifact lines** the probable future shape, *not* built in B3); a **per-edge kill-switch** (the principled inverse of B2 — off = a safe baseline, off≠the vuln); **virtual-time tests** (injectable clock). Zero `opa-abac-core`/Rego change; B2's contract retained. Java 21 / Boot 3.4 baseline. Load-testing rig deferred to **Phase 7 polish**. Source: [[B2-SUPPLIER-OUTAGE]] (00-DESIGN §5) + ADR [[0014-supplier-outage-error-distinct|0014]] (§Consequences). |
