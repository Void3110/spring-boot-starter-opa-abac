...
- ✅ **BUILT 2026-09-11 (collaborative, five tickets; PR pending):** [[TAG-GATED-CREATE]] ([[00-DESIGN]], [[0034-tag-gated-placement-input-contract|ADR 0034]]) — DEF-1 of the pre-Habr UI QA: a role holding WRITE **and** a tag requirement can `create` categories and products it is denied to read, because the type-level create gate (`list_inheritable_grant`) checks the verb and never the tags. Decided: **placement by tags** — the placement parent's tags (root included, both flag states) **and** the payload must satisfy the role; re-parent is a second placement call; LIST stays the only coarse type-level verb; `parent_attributes` (three-state, no fallback) + three `@OpaPreAuthorize` attributes, manager-side. Small collaborative slice, three-tier ratchet.
