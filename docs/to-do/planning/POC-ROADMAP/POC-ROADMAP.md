...
zero Rego. Direction is set ([[RESOURCE-RESOLUTION]]); it sequences **before** Phase 6, which consumes
...
and hierarchy. Direction is set ([[ACTION-ENRICHMENT]]); decomposition follows once 5.97 ships.
...
- Phase 5.95 shipped: ADR [[adr/0012-pagination-envelope|0012]] + [[PAGINATION-ENVELOPE]] — the exact-count pagination envelope composed with the partial-eval filter (Epic D5 in [[USER-STORIES]]); next: 5.97 → Phase 6 action enrichment lands on this envelope
- Phase 5.97 direction: [[RESOURCE-RESOLUTION]] — attribute-rich pre-authorization (`AbacResourceResolver` SPI + request-scoped resource cache); prerequisite for Phase 6
- Phase 6 direction: [[ACTION-ENRICHMENT]] — affordance metadata via batch eval + an `x-implements` marker, evaluated on 5.97-resolved attributes
