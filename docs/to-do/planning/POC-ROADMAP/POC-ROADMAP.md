...
> **Route to publish (settled 2026-06-13).** With 6.5 merged (PR #34), the order to Phase 7 is
> **B2 fix-slice → Phase 6.7 → Phase 6 → Phase 7 (publish)** — this **reorders the old `6 → 6.7`**:
> 1. **B2 fix-slice** *(standalone, first)* — the supplier-outage error-distinct posture. A resolve
>    outage currently rides the realm-role fallback and, post-6.5, **erases** the resolved role's
>    `denied_actions`/`required_tags` narrowing (the one tracked widening-on-failure, [[PERMISSION-MODEL]]
>    / the 6.5 review). Fixed on its own small branch — distinguish *outage → deny* from *no-role →
>    fallback* at the SPI contract — kept standalone (its own design + review) so the security fix
>    isn't entangled with 6.7's larger taxonomy work.
...
> 4. **Phase 7 — publish prep.** The small tracked items (OPA authn/trust docs · CI-runs-e2e · runner
...
| **B2** | **Supplier-outage fix-slice** *(security; route step 1)* | Distinguish a resolve **outage** (→ deny) from an authoritative **no-role** (→ realm fallback) at the `RoleDefinitionSupplier`/SPI contract, so a user-mgmt outage can no longer ride the catalog realm fallback to a **wider** grant than the resolved role — a hole **aggravated by 6.5** (an outage erases the resolved role's `denied_actions`/`required_tags` narrowing). The one tracked widening-on-failure path. | 🔜 **Next (route step 1, settled 2026-06-13).** Standalone small branch with its own phase-① + review (kept separate from 6.7's taxonomy work). Source: [[PERMISSION-CATEGORIES-REVIEW]] (C1/C4), retro-audit 2026-06-12 ("supplier-outage error-distinct posture — an SPI-contract change, needs its own design pass"). The supplier javadoc/log already state the posture honestly; this slice changes the behavior. |
