...
| **5.9** | **REST API refinement (publication-readiness)** | Close the maturity gaps the REST API design review found before the library is published — **no fail-open exists**, these are the distance between "clean demo" and "a published library's reference services". Headline: **RFC-7807 `problem+json` + a machine-stable `errorCode` enum** (so clients branch on a code, not a localizable message). Plus the cheap wins: a `Location` header on every `201`; one-line intent comments at the deliberately-ungated user-service bootstrap mutations (so an absent `@OpaPreAuthorize` reads as a decision, not a forgetting). A shared **pagination envelope** is noted but likely deferred (a library-wide slice when volume justifies it). Sequenced **before Phase 6** so the action-enrichment `_actions` envelope lands on an already-RFC-7807-clean surface. | 🔜 **NEXT — to plan.** Source: [[REST-API-DESIGN-REVIEW]] (findings 1–5) against the guide [[REST-API-DESIGN]]. Needs the planning ritual (grill-me to settle the slice scope — esp. how far to take problem+json + whether pagination is in or deferred — then an ADR for the error-contract decision + the slice-planner decomposition). Affordance metadata (review finding 6) stays **Phase 6**. |
| **6** | **Action enrichment (affordance metadata)** | After a handler returns a resource/page, a response decorator attaches an `_actions` map — *which actions the caller may perform on it* — so a UI renders exactly the right buttons. Powered by Phase-5 **batch eval** (one OPA round-trip → N action verdicts), an **action registry** per resource type (also a validation allowlist), and an **`x-implements` marker** (`Enrichable`) stamped onto the generated DTOs via the OpenAPI generator. Affordance, **not** enforcement. | 🔜 **Planned (direction set, not yet decomposed)** — [[ACTION-ENRICHMENT]]. First real consumer of the Phase-5 `allowAll` primitive (so it sequences right after Phase 5.9). Open question: the `_actions` envelope ↔ OpenAPI codegen fit (the `x-implements` marker, modeled on the source platform's `AutoCloseable` DTO precedent). ADR pending, to be written with its decomposition. |
