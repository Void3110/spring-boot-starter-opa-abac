...
- Phase 10 ✅ **COMPLETE** (phase ① settled 2026-08-01; slices A/B/C shipped 2026-08-07/13/15, and the console follow-up **[[SPA-CHALLENGE-UX]] ✅ SHIPPED 2026-08-18** — mini package, collaborative build, closed by a committed E10–E21 browser case list and a three-round layer-3 review; **the 1.2.0 cut is now unblocked**): **the supervisor read path** — a unit manager who is a member of **no** team sees the catalogs of the teams their reports own or manage, read-only, at production detail behind a second factor. Pinned by [[0029-supervised-read-scope|ADR 0029]] (scope: a second **disjoint** access path — membership always wins so `supervised := S \ M`, CONTROL-capable reach, a new fail-closed org-relation seam, a synthesized read-only role carrying provenance, a two-leg partitioned list) and [[0030-step-up-decision-contract|ADR 0030]] (elevation: no new verb — child reads gated on the supervised path only; an `operatorManaged` tag flag making the `env` tier unstrippable; resource-server-side `auth_time` freshness as the whole control, **measured** on the rig; an additive `deny_reason` and an RFC 9470 challenge). **Failed the slice-sizing gate as one slice** (~13 tickets over five deployables) → ships as three, each fail-closed at its boundary so every later slice only *widens*: **A** [[SUPERVISED-SCOPE]] (✅ **SHIPPED 2026-08-07** — the list, read-only, contents closed; 6 tickets, two example services + the policy corpus, no library change, one narrow Rego change — ADR 0031's inheritance confinement), **B** [[PRODUCTION-TIER]] (✅ **SHIPPED 2026-08-13** — the operator-managed `env` tier: non-prod contents open by direct grant, production/unproven closed by provenance-scoped denies; 6 tickets, two-part orchestrated run + layer-3 review, PR #105), **C** [[STEP-UP-ELEVATION]] (✅ **SHIPPED 2026-08-15** — the RFC 9470 round trip: one TOTP opens production contents for a bounded `auth_time` window, refresh cannot stretch it, both audit events live, the supervised path closed to agent calls; 6 tickets, two-part orchestrated run + 18-round layer-3 review, PR #107), then **the console follow-up** [[SPA-CHALLENGE-UX]] (✅ **SHIPPED 2026-08-18** — design settled 2026-08-15 — the SPA consumes the RFC 9470 challenge: inline locked panel, full-page redirect re-auth with the miner's recipe, one retry, a learned-window elevation chip, amber-predicts/server-decides badges over a new `_provenance` affordance ([[0033-catalog-provenance-affordance|ADR 0033]]), a `sup-demo` persona, a committed Browser-pane case list). The SPA's challenge UX stays collaborative rather than autonomous
- Phase 11 📋 **DIRECTION** (2026-08-06, sequenced right after Phase 10 A/B/C): **Keycloak capability projection** — a new optional `opa-abac-keycloak-projection` module (sibling of `opa-abac-keycloak-directory`, same admin-client plumbing + fail-closed doctrine) that **materializes the ABAC model's native-enforceable subset into Keycloak group memberships**, so RBAC-only consumers — GitOps control planes (Kargo, Argo CD), Kubernetes RBAC, Grafana-class tools — enforce the *same source of truth* via their own `groups`-claim bindings. The recognized architecture (AuthZed Materialize's precompute-and-stream pattern, hand-implemented into Keycloak groups) with no existing Java implementation. Library half = a pure projector (role assignments × the `permission_categories` expansion ∩ a configured projectable subset → group names under a configurable grammar) + an idempotent applier + a reconciler-as-a-callable with a drift hook; eventing/scheduling stays app-side (the `RoleDefinitionSupplier` division). **Load-bearing doc rule**: projection feeds RBAC-only consumers — the OPA gate stays on live attributes, or a projector bug defeats every layer at once. Candidate headline proof: a **minikube profile** whose apiserver OIDC-trusts the rig Keycloak and whose RoleBindings reference projected groups — a kubectl allow/deny matrix as the "enforced by a system that never heard of OPA" demo, possibly with a small example client app. Phase ① (grill-me over the direction agenda) not started.
- 📋 **RESEARCHED (2026-09-07), to be designed:** [[GATEWAY-AUDIENCE-BINDING]] — the example rig checks a token's signature, expiry and issuer at the gateway but **never its audience**, and the realm mints no audience claim, so every client's token is interchangeable (a Keycloak-admin service-account token would pass any route). Decided: **bump APISIX 3.11.0 → 3.18.0 first** (its `claim_validator` binds `aud` and replaces the Lua issuer guard), then a Keycloak audience mapper + e2e pins + the adopter warning in the starter's `trust-forwarded-jwt` docs; **ADR 0035** (0034 went to TAG-GATED-CREATE); collaborative mini-slice.
- 📐 **DESIGNED (2026-09-11), to be decomposed + built collaboratively:** [[TAG-GATED-CREATE]] ([[00-DESIGN]], [[0034-tag-gated-placement-input-contract|ADR 0034]]) — DEF-1 of the pre-Habr UI QA: a role holding WRITE **and** a tag requirement can `create` categories and products it is denied to read, because the type-level create gate (`list_inheritable_grant`) checks the verb and never the tags. Decided: **placement by tags** — the placement parent's tags (root included, both flag states) **and** the payload must satisfy the role; re-parent is a second placement call; LIST stays the only coarse type-level verb; `parent_attributes` (three-state, no fallback) + three `@OpaPreAuthorize` attributes, manager-side. Small collaborative slice, three-tier ratchet.
