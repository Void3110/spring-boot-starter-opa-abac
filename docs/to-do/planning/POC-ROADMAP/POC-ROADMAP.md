...
> **reorders the old `6 → 6.7`** and adds **B3** before publish. **Status (2026-06-18): B2, 6.7,
> Phase 6 (action enrichment), and now B3 (cross-service HTTP resilience) are all ✅ shipped** (B3 on
> branch `feature/void3110/http-resilience`, T1–T4, the live e2e headline passed). Remaining order:
> **Phase 7 (publish)** — B3 was the last slice before publish.
...
- Phase 7.3 (pre-publish) **resolve coalescing** [[RESOLVE-COALESCING]] — **design settled + decomposed** (grill-me + `/decompose` 2026-07-10, T1–T6 ready for the autonomous run; pinned by [[0023-request-scoped-resolution-memoization|ADR 0023]] + [[0024-batch-role-resolution|ADR 0024]]): the 7.2 findings consumed — a request-scoped **memo** over the role + ancestor SPIs (all three tri-state outcomes, "one request, one answer per target", single default-on flag `opa.abac.resolve-memo.enabled`) collapses the measured 2/22/102 same-target resolves to 1; a **batch `lookupAll`** (two-state entries, whole-batch outage, strict completeness, one guarded internal GET) collapses the memo-proof **multi-root** pages (the catalogs list); plus the tail — APISIX `opa` plugin `timeout:500` (int ms), breaker/sampling adopter notes, the list batch-eval bound **re-pinned to 2**, and a new multi-root k6 scenario whose baseline lands **before** the fix (honest before/after). Runs **before** the SB4 port; the memo makes the **7.4 stale-authorization scrutiny mandatory** (staleness contract: one request, one answer)
