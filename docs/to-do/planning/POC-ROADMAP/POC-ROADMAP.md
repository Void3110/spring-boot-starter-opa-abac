...
> **Route to publish (settled 2026-06-13; B3 inserted 2026-06-15).** The order to Phase 7 is
> **B2 fix-slice → Phase 6.7 → Phase 6 → B3 resilience → Phase 7 (publish)** — this
> **reorders the old `6 → 6.7`** and adds **B3** before publish. **Status (2026-06-17): the two
> correctness slices before Phase 6 — B2 and 6.7 — are both ✅ shipped; Phase 6 (action enrichment) is
> now the front of the queue (direction set, not yet decomposed).** Remaining order: **Phase 6 → B3 → Phase 7.**
...
> 3. **Phase 6 — action enrichment** *(after 6.7 — ◀ NOW NEXT)* — a fuller feature story before
>    publish (affordance metadata is the headline differentiator). Independent of 6.7; consumes 5.97
>    attributes + the 6.5/6.7 vocabulary. Direction set ([[ACTION-ENRICHMENT]]); **next move is
>    phase-① grill-me → /decompose** (open: the `_actions` envelope ↔ OpenAPI codegen / `x-implements`
>    fit; an ADR is still pending).
...
| **6** | **Action enrichment (affordance metadata)** | After a handler returns a resource/page, a response decorator attaches an `_actions` map — *which actions the caller may perform on it* — so a UI renders exactly the right buttons. Powered by Phase-5 **batch eval** (one OPA round-trip → N action verdicts), an **action registry** per resource type (also a validation allowlist), and an **`x-implements` marker** (`Enrichable`) stamped onto the generated DTOs via the OpenAPI generator. Affordance, **not** enforcement. | 🔜 **NEXT — direction set, not yet decomposed; all upstream slices shipped (B2 + 6.7 done 2026-06-15).** [[ACTION-ENRICHMENT]]. First real consumer of the Phase-5 `allowAll` primitive (so it sequences after Phase 5.9 + 5.95 — the `_actions` envelope lands on an already-paginated, RFC-7807-clean surface). **Also consumes the 5.97 resolver/cache** ([[RESOURCE-RESOLUTION]]): each `_actions` verdict is evaluated against **fully resolved resource attributes**, so affordance mirrors enforcement — incl. tag grants/denies and hierarchy. **And after 6.5/6.7**: the action registry + `_actions` keys enumerate the fine-action vocabulary 6.5 defines, **plus the categorized control-plane verbs from 6.7** (route order **B2 → 6.7 → 6 → B3 → 7**, settled 2026-06-13 — this **reordered** the earlier 6→6.7). **Next move: phase-① grill-me → /decompose.** Open question: the `_actions` envelope ↔ OpenAPI codegen fit (the `x-implements` marker, modeled on the source platform's `AutoCloseable` DTO precedent). ADR pending, to be written with its decomposition. |
