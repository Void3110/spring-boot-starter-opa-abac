...
- Phase 7 (pre-publish) **user-directory work, split into two slices** (grill-me 2026-07-06): **Slice 1** [[DIRECTORY-QUERY-FILTERS]] — **decomposed, ready for the autonomous run** (5 tickets: `?subject`/`?targetType`+`?targetId` filters that kill the SPA's `listAll*` page-walks, the 204/`Accept` `produces` fix, the bootstrap displayName upsert); **Slice 2** (Keycloak-admin `UserDirectory` port + optional module + the `search` endpoint under `/api/v1/users`) — designed, decomposed next
