...
- Phase 7 (pre-publish) **user-directory work, split into two slices** (grill-me 2026-07-06): **Slice 1** [[DIRECTORY-QUERY-FILTERS]] — **shipped** (2026-07-07, the autonomous run): the `?subject`/`?targetType`+`?targetId` exact-match filters (one-item page, both-or-400) killed the SPA's `listAll*` single-resource page-walks, plus the 204/`Accept` `produces` fix and the bootstrap displayName upsert; **Slice 2** [[USER-DIRECTORY-PORT]] — **decomposed, ready for the autonomous run** (6 tickets: the `UserDirectory` search SPI in the library, a `KeycloakUserDirectory` in a new optional module, starter auto-config, the bearer-only `search` endpoint under `/api/v1/users`, the least-privilege `view-users` realm client, e2e + SPA picker rewrite; pinned by [[0020-user-directory-port|ADR 0020]])
