...
T5's: an e2e persona *is* a Keycloak user, so the new persona accounts and the UX-only `unit-supervisor`
...
6. **Context you will be checked against** in the review gate (step 5):
   ...
   (the classification discipline T3 mirrors),
   `.../config/HttpRoleDefinitionSupplier.java` and `.../config/TagDefinitionClient.java` (the shipped
   `CallGuard` edge wrapping), `.../config/CatalogListAuthorizer.java` (what T4 rewrites),
   ...
   `.../service/EffectiveRoleService.java` and `.../service/TeamRoleCapabilities.java` (T2's seam and the
...
9. **Prime Mulch:** `ml prime opa-abac-authz-model spring-security-integration --budget 8000`, then
   `ml prime opa-abac-e2e-suite --budget 8000` before T5. Directly relevant records: **mx-6f551d**
...
### Per-ticket loop (tickets T1 → T5, IN ORDER — strictly sequential; T1+T2 are the standalone-value subset)
...
2. **Build the deliverables** exactly as `01-DECOMPOSITION.md` lists them — exact classes, packages
   (`dev.dmitriikonovalov.example.usermgmt.{domain,service,web}` for T1–T2,
   `dev.dmitriikonovalov.example.catalog.config` for T3–T4), mappings, rego rules. Match the surrounding
...
3. **Write/extend the tests** for the ticket (the relevant U*/I*/E* cases from `10-QA-TEST-CASES.md`).
   ...
   `opa test` — **this slice edits no policy, so `opa test` must stay green and unchanged.**
...
6. **Integration / e2e validation (MANDATORY for the relevant tickets).**
   ...
   - T5: bring the rig up (`ENABLE_OIDC=1 ENABLE_USER_SERVICE=1 ./deploy.sh up --pods 2`), then
     ...
     resolution on this one rig, and **record in `STATUS-05.md` exactly which you ran and which you
...
- **Slice-specific invariants — never trade these away:**
  **(1) Membership always wins** — `supervised := S \ M`; the two scopes stay disjoint, which is what
  ...
  verb expands to ∅ and grants nothing at all); the missing child keys are what keep contents closed with
  zero Rego edits. **(5) Additive
  only** — no library module changes, no Rego changes, no decision-envelope changes. **(6) A partial
...
- **The headline tickets** — **T4** (the two-leg partitioned list: the mechanism the whole slice exists
  for, and the one place a mistake widens rather than narrows) and **T5** (E1 + E4: the derived scope is
...
- **Rig / e2e specifics** — `ENABLE_OIDC=1 ENABLE_USER_SERVICE=1 ./deploy.sh up --pods 2`. Honor the
  in-network token caveat. No OPA restart is needed for this slice (**zero policy edits** — if you find
  yourself restarting OPA for a policy change, you have left the slice boundary). E8 needs a
  fault-injected supervised edge — reuse B3's approach (`mx-91fa5d`), do not invent a second mechanism.
